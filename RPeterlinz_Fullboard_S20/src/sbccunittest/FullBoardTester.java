...
public class FullBoardTester {
	...
	public void testPmd() {
		try {
			execPmd("src" + File.separator + "fullboard", "cs106.ruleset");
		} catch (Exception ex) {
			fail(ex.getMessage());
		}
	...
	private static String trimFullClassPaths(String output) {
		...
		for (String line : lines)
			sb.append(substringAfterLast(line, File.separator)).append(getProperty("line.separator"));
